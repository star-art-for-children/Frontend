...
test.describe('홈 페이지', () => {
  ...
  test('정렬 필터 버튼이 모두 표시된다', async ({ page }) => {
    ...
    await expect(
      page.getByRole('button', { name: '예정된 전시' })
    ).toBeVisible();
    await expect(
      page.getByRole('button', { name: '종료된 전시' })
    ).toBeVisible();
  });
  ...
  test('검색어 입력 후 제출 시 URL search 파라미터가 추가된다', async ({
    page,
  }) => {
    ...
    await expect(page).toHaveURL(
      /search=%EB%B4%84\+%EC%A0%84%EC%8B%9C|search=.*봄/
    );
  });

  test('검색어를 비우고 제출하면 search 파라미터가 제거된다', async ({
    page,
  }) => {
  ...
  test('헤더에 로그인 / 회원가입 링크가 표시된다 (비로그인)', async ({
    page,
  }) => {
